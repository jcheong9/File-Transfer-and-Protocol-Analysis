...
void serverMainUDP(PVOID network)
{
    ...
    if ((ListenSocketUDP = WSASocket(AF_INET, SOCK_DGRAM, 0, NULL, 0,
    ...
    memset((char*)&InternetAddr, 0, sizeof(InternetAddr));
    InternetAddr.sin_family = AF_INET;
    InternetAddr.sin_addr.s_addr = htonl(INADDR_ANY);
    InternetAddr.sin_port = htons(SERVER_PORT);
    // Bind an address to the socket
    if (bind(ListenSocketUDP, (struct sockaddr*)&InternetAddr,
    ...
    */
    ...
    if ((ThreadHandle = CreateThread(NULL, 0, WorkerThreadUDP, 0, 0, &ThreadId)) == NULL)
    {
    ...
    {
        AcceptSocketUDP = accept(ListenSocketUDP, NULL, NULL);
    ...
    while (TRUE)
    {
        ...
        */
        ...
        if (WSARecvFrom(SocketInfo->Socket, &(SocketInfo->DataBuf), 1, &RecvBytes, &Flags,
            (SOCKADDR*)&client_UDP, &clientAddrSize , &(SocketInfo->Overlapped), NULL) == SOCKET_ERROR)
        {
